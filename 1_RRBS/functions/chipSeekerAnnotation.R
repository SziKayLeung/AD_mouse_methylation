...
ChipAnnotatePeaks <- function(File,merge=NULL){
  ...
  if(nrow(File) != 0){
    ...
    if(!is.null(merge)){
      ...
      File <- File %>% dplyr::select(-commonCol) %>% mutate(position = Position)
      peakAnno.df <- merge(File,peakAnno.df, by = "position")
    }
...
ChipFinaliseAnno <- function(peakAnno.df, peakAnno.df_intronexon, stat.df){
  ...
  stat.df <- stat.df %>% mutate(position = Position)
...
ChipRenameCol <- function(peakAnno.df){
  
  if(!is.null(peakAnno.df)){
...
# Input:
  # df = output from () after running chipseeker; contains annotation column 
# Output:
