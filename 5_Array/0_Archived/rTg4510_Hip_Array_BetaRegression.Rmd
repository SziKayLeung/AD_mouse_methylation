---
title: "rTg4510 Hippocampus Array Beta Regression Results"
output: 
  html_document:
    code_folding: hide
    toc: true
...
All samples are from Hippocampus region. There are 39 samples, 20  WT, 19 TG. Except TG 8months with 4 samples, all other ages in both genotypes have 5 samples. 
...
#                            link = "probit")
# 
# save(betaResultsChip, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/ArrayBetaRegResults_rTg4510_HIP.RData")
```
...
# write.csv(betaResults, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_stats_table_rTg4510_HIP.csv")
# 
# 
# sig_genotype <- betaResults[which(betaResults[,"FDR_adj_genotype"] < 0.05),]
# write.csv(sig_genotype, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_rTg4510_HIP_sig_genotype.csv")
# 
# sig_age <- betaResults[which(betaResults[,"FDR_adj_age"] < 0.05),]
# write.csv(sig_age, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_rTg4510_HIP_sig_age.csv")
# 
# sig_interaction <- betaResults[which(betaResults[,"FDR_adj_Interaction"] < 0.05),]
# write.csv(sig_interaction, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_rTg4510_HIP_sig_interaction.csv")
...
manhattan(betaResults[2:nrow(betaResults), ], p = "FDR_adj_genotype", bp = "Bp", chr = "Chr", 
          genomewide = -log10(threshold), suggestiveline = FALSE, 
          logp=T, col = c("black", color_Tg4510_TG), main = "Genotype",
          chrlabs = c(1:19, "X", "Y"),las = 2, annotatePval = threshold,
          cex.axis = 1.5, cex.lab = 1.2)
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/rTg4510_HIP_Genotype.pdf", onefile = T)
for(i in 1:6){
  plotDMPGenotype(cpg = rownames(betaResults[i,]) , color = color_Tg4510_TG, betaResults = betaResults,
                   betas = betas, pheno = pheno)
}
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/rTg4510_HIP_Interaction.pdf", onefile = T)
for(i in 1:6){
  plotDMPInteraction(cpg = rownames(betaResults[i,]) , color = color_Tg4510_TG, betaResults = betaResults,
                   betas = betas, pheno = pheno)
}
...
#                                                      link = "probit")
# resave(betaResultsPathology, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/ArrayBetaRegResults_rTg4510_HIP.RData")
```
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/rTg4510_HIP_Pathology.pdf", onefile = T)
for(i in 1:6){
  plotDMPPathology(cpg = rownames(betaResults[i,]) , color = color_Tg4510_TG, betaResults = betaResults,
                   betas = betas, pheno = pheno)
}
...
# write.csv(betaResults, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_stats_table_rTg4510_HIP_Pathology.csv")
# 
# 
# sig_pathology <- betaResults[which(betaResults[,"FDR_adj_pathology"] < 0.05),]
# write.csv(sig_pathology, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_rTg4510_HIP_sig_pathology.csv")
...
#                            link = "probit")
# 
# save(betaResultsChip, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/ArrayBetaRegResultsFull_rTg4510_HIP.RData")
...
load("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/ArrayBetaRegResultsFull_rTg4510_HIP.RData")
betaResults <- betaResultsChip
# Add chr and bp before deleting rows
Chr <- species_probes$Chr[match(species_probes$probeID, betaResults[,"cpg"])]
Bp <- species_probes$Bp[match(species_probes$probeID, betaResults[,"cpg"])]
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/FullrTg4510_HIP_Genotype.pdf", onefile = T)
for(i in 1:6){
  plotDMPGenotype(cpg = rownames(betaResults[i,]) , color = color_rTg4510_TG, betaResults = betaResults,
                   betas = betas, pheno = pheno)
}
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/FullrTg4510_HIP_Pathology.pdf", onefile = T)
for(i in 1:6){
  plotDMPPathology(cpg = rownames(betaResults[i,]) , color = color_rTg4510_TG, betaResults = betaResults,
                   betas = betas, pheno = pheno)
}
...
write.csv(betaResults, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_stats_table_FullrTg4510_HIP.csv")


sig_genotype <- betaResults[which(betaResults[,"FDR_adj_genotype"] < 0.05),]
write.csv(sig_genotype, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_FullrTg4510_HIP_sig_genotype.csv")

sig_age <- betaResults[which(betaResults[,"FDR_adj_age"] < 0.05),]
write.csv(sig_age, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_FullrTg4510_HIP_sig_age.csv")

sig_pathology <- betaResults[which(betaResults[,"FDR_adj_pathology"] < 0.05),]
write.csv(sig_pathology, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/DMPs_FullrTg4510_HIP_sig_interaction.csv")
