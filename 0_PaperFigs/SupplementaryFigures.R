...
pAnk1DMP <- plotGeneTrackDMP(sigResults=rTg4510_ECX_sigResultsDMPs$Genotype, betaMatrix=rTg4510_sig_beta$Genotype, phenotypeFile=phenotype$rTg4510, 
                             gene="Ank1", transcript="ENSMUST00000110688.8")
ank1PyroPos <- c(
...
pAnk1Pyro <- input_pyro$ank1 %>% dplyr::select(Age, Sample.group, contains("Pos")) %>% 
  reshape2::melt(id = c("Age","Sample.group"), variable.name = "Position", value.name = "methylation") %>% 
