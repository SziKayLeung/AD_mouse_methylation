...
rm(betaResultsNull, predictedMethNull)
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/UniqueProbes_ComparingSmoothedMethylationToArray.pdf",w = 12)
...
write.csv(cor_matrix, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/smoothed_correlation.csv")
#######################################################################################################
#######################################        2)Non-smoothed       ###################################
#######################################################################################################
# Load the  library & data in
...
library(reshape2)
library(ggpubr)
load("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/biseq_rTg4510.RData")
...
QCmetrics$sampleid <- sampleid
colnames(Normalised_Sesame_Betas) <- sampleid
Array_betas <- Normalised_Sesame_Betas[,QCmetrics[which(QCmetrics$Tissue == "CortexEntorhinalis" & QCmetrics$AD_model == "rTg4510"), "sampleid"]]
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/UniqueProbes_ComparingNonSmoothedMethylationToArray.pdf",w = 12)
...
write.csv(cor_matrix, file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/nonsmoothed_correlation.csv")


#######################################################################################################
#########################           3) Compare the correlations           #############################
#######################################################################################################
nonsmoothedcorr <- read.csv("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/nonsmoothed_correlation.csv", header= T, row.names = 1)
smoothedcorr <- read.csv("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/smoothed_correlation.csv", header= T, row.names = 1)
...
pdf("/gpfs/ts0/projects/Research_Project-191406/Aisha/data/Array/Comparison_Smoothed_NonSmoothed.pdf", w = 12)
...
ggplot(CorrelationMatrices , aes(x = Smoothed, y = NonSmoothed))+
  ...
  theme_classic() +
  stat_cor(method="pearson") +
  labs(title = "1201 sites", x = "Smoothed", y = "Non-Smoothed") +
  theme(plot.title = element_text(hjust = 0.5))  
