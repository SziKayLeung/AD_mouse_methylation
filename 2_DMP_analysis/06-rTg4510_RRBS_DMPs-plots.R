# title: "RRBS - BiSeq rTg4510 - individual plots DMPs"
# author: "Isabel Castanho (I.S.Castanho@exeter.ac.uk)"

load(file = "/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/biseq_predictedMeth_rTg4510.RData") # predictedMeth
...
ResultsGenotype <- read.csv("/lustre/projects/Research_Project-191406/isabel/RRBS_new/DMPs/rTg4510/DMPsInteractionModel_rTg4510_sig_genotype.csv",
...
AnnotatedResultsGenotype <- read.csv("/lustre/projects/Research_Project-191406/EmmaW/RRBSAnnotatedResults/rTg4510/DMPs/DMPsInteractionModel_rTg4510_sig_genotype_1500bptssAnno.csv",header = TRUE)
...
write.csv(FilteredAnnotatedResultsGenotype,
          file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/Annotated/rTg4510/rTg4510-DMPsGenotype_annotatedMethylDiffFiltered.csv")
...
dataPlot <- DF[1:500,]
sites_to_plot <- dataPlot$betaResultsClean....Position..
genes_to_plot <- as.character(dataPlot$biomart_Nearest_gene)

pdf(file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/IndividualPlots/rTg4510/rTg4510_GenotypeTop500FDR_MethylDiffFiltered.pdf", width=5, height=10)
for (index in 1:length(sites_to_plot)) {
  ...
  boxplot(data_plot[,as.character(site)] ~ data_plot$Genotype,
          axes=FALSE, ann=FALSE,
          col = c("grey", color_Tg4510_TG)[data_plot$Genotype],
          add = TRUE, at = 1:2)
}
dev.off()
...
## Remove sites with methylation difference < 0.02 (2%) between all WT timepoint 4 TG (all WT and 8 months TG)
ResultsInteraction <- read.csv("/lustre/projects/Research_Project-191406/isabel/RRBS_new/DMPs/rTg4510/DMPsInteractionModel_rTg4510_sig_interaction.csv",
                               header = TRUE, row.names = 1)
colnames(ResultsInteraction)[1] <- "Location"

## Get methylation mean difference data
load(file = "/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/rTg4510_DMPsInteraction_meanDiffTG-WT.RData")
# DatFrame
...
## Get annotated tables
AnnotatedResultsInteraction <- read.csv("/lustre/projects/Research_Project-191406/EmmaW/Annotated/rTg4510/DMPs/With_3000bpTSS_Annotations/DMPsInteractionModel_rTg4510_sig_interaction_3000tssAnno.csv",
                                        header = TRUE)
...
write.csv(FilteredAnnotatedResultsInteraction,
          file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/Annotated/rTg4510/rTg4510-DMPsInteraction_annotatedMethylDiffFiltered.csv")
...
pdf(file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/IndividualPlots/rTg4510/rTg4510_InteractionTop500FDR_MethylDiffFiltered.pdf", width=5, height=10)
for(index in 1:length(sites_to_plot)) {
  site <- sites_to_plot[index]
  gene <- genes_to_plot[index]
  data_plot <- cbind(coldata[,c("Age_months","Genotype", "Group_ID")], t(data[as.character(site),]))
...
## Remove sites with methylation difference < 0.02 (2%) between all WT timepoint 4 TG (all WT and 8 months TG)
ResultsPathology <- read.csv("/lustre/projects/Research_Project-191406/isabel/RRBS_new/DMPs/rTg4510/DMPsPathology_rTg4510_sig_pathology.csv",
                             header = TRUE, row.names = 1)
colnames(ResultsPathology)[1] <- "Location"

## Get methylation mean difference data
load(file = "/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/rTg4510_DMPsPathology_meanDiffTG-WT.RData")
# DatFrame
...
## Get annotated tables
AnnotatedResultsPathology <- read.csv("/lustre/projects/Research_Project-191406/EmmaW/Annotated/rTg4510/DMPs/With_3000bpTSS_Annotations/DMPsPathology_rTg4510_sig_pathology_3000tssAnno.csv",
                                      header = TRUE)
...
write.csv(FilteredAnnotatedResultsPathology,
          file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/Annotated/rTg4510/rTg4510-DMPsPathology_annotatedMethylDiffFiltered.csv")
...
pdf(file = "/lustre/projects/Research_Project-191406/isabel/RRBS_new/IndividualPlots/rTg4510/rTg4510_PathologyTop500FDR_MethylDiffFiltered.pdf", width=5, height=10)
for(index in 1:length(sites_to_plot)) {
  site <- sites_to_plot[index]
  gene <- genes_to_plot[index]
  data_plot <- cbind(coldata[,c("Age_months","Genotype", "Group_ID")], t(data[as.character(site),]))
...
pdf(file = "/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/ScatterPlots/rTg4510/rTg4510_PathologyTop500FDR_scatterPlots_Pathology.pdf", width=10, height=10)
for (index in 1:length(sites_to_plot)) {
  site <- sites_to_plot[index]
  gene <- genes_to_plot[index]
...
input <- list(
  rTg4510_geno = read.csv(paste0(dirnames$output,"rTg4510_rrbs_genotype.csv"))
)

# plot function
plotMeth <- function(site,coldata,data,data_plot){
  
  gene <- data_plot[data_plot$Position == site, "ChIPseeker_GeneSymbol"]
  print(gene)
  data_plot <- cbind(coldata[,c("Age_months","Genotype", "Group_ID")], t(data[as.character(site),]))
  data_plot <- data_plot[complete.cases(data_plot), ]
  #dev.new(width=5, height=10)
  ...
  axis(2, cex.axis=2)
  title(main = paste(gene, " ", site), ylab = "Methylation (%)", cex.main=2, cex.lab=2)
  p <- boxplot(data_plot[,as.character(site)] ~ data_plot$Genotype,
               axes=FALSE, ann=FALSE,
               col = c("grey", color_Tg4510_TG)[data_plot$Genotype],
               add = TRUE, at = 1:2)
...
pdf(file = paste0(dirnames$output, "Ank1_scatterPlots_Pathology.pdf"), width=10, height=10)
plotMeth("chr8:23023193",coldata,data,input$rTg4510_geno)
plotMeth("chr8:23023211",coldata,data,input$rTg4510_geno)
plotMeth("chr8:23023240",coldata,data,input$rTg4510_geno)
plotMeth("chr8:23023241",coldata,data,input$rTg4510_geno)
dev.off()