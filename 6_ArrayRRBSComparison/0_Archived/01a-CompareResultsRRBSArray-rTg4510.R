...
## load rrbs
load("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/biseqResultsGenotype_rTg4510.RData")
...
load("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/biseqResultsInteraction_rTg4510.RData")
...
save(GenotypeResults, InteractionResults ,file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/CompareTech/rTg4510/TechComparison_rTg4510.RData")


## If you want to increase data points for comparison, compare rrbs site 50bp+- to the array

# what is the minimum distance between shared sites in techs for rrbs
# This will let us know how many sites overlap depending on the window selected. Overlapping positions cannot be given a window 
tmp <- GenotypeResults[,c(1,14:23)]
chrs <- unique(GenotypeResults$R.chr)
distdf <- matrix(NA, nrow = 1, ncol = 2)
distdf <- as.data.frame(distdf)
colnames(distdf) <- c("position","Diff")
for(i in chrs) {
  tmpchr <- tmp[which(tmp$R.chr == i),]
  tmpchr <- tmpchr[order(tmpchr$R.pos), ]
  tmpchr <- tmpchr %>%
    mutate(Diff = R.pos - lag(R.pos)) #this works out the difference of cpgs on each chrs
  tmpchr2 <- tmpchr[,c(1,12)]
  distdf <- rbind(distdf, tmpchr2)
}
distdf <- distdf[-1,]
rownames(distdf) <- c(1:nrow(distdf))
length(which(distdf$Diff < 10)) #
rmsharedsits <- GenotypeResults[which(GenotypeResults$position %in% distdf[which(distdf$Diff < 10),1]),]




## choose 50bp window of the 1201 rrbs positions
window = 10
data <- RRBSGenotypeResults[!c(RRBSGenotypeResults$position %in% rmsharedsits$position),]
rrbspos <- GenotypeResults$position
rrbschr <- gsub(":.*","", rrbspos) 
rrbspos <-gsub(".*:","", rrbspos) 
data2 <- rep(NA, ncol(data))
clump <- c()
for(i in 1:length(GenotypeResults$position)){
  chr <- as.numeric(rrbschr[i])
  start <-  as.numeric(rrbspos[i]) - window
  end <-  as.numeric(rrbspos[i]) + window
  index <- which(data$R.chr == chr & data$R.pos <= end & data$R.pos >= start )
  clump <- c(clump, rep(GenotypeResults$position[i], length(index)))
  data2 <- rbind(data2,data[index,])
}
data2 <- data2[-1,]
data3 <- cbind(data2, clump)
rmsharedsits <- rmsharedsits[,c(14:23,1)]
rmsharedsits$clump <- rmsharedsits$position
data4 <- rbind(rmsharedsits, data3)
rownames(data4) <- c(1:nrow(data4))

## Can we know bind the windows RRBS data with the array?
colnames(data4)[colnames(data4) == 'position'] <- 'position.acc'
colnames(data4)[colnames(data4) == 'clump'] <- 'position'
GenotypeResults2 <- merge(ArrayGenotypeResults, data4, by = "position") 
R.FDR_adj_genotype <- p.adjust(GenotypeResults2[,"R.p.val"], method = "fdr")
GenotypeResults2$R.FDR_adj_genotype <- R.FDR_adj_genotype
rm(R.FDR_adj_genotype)

Geno <- GenotypeResults2[which(GenotypeResults2$A.FDR_adj_genotype <= 0.05 & GenotypeResults2$R.FDR_adj_genotype <= 0.05),]

A.SignGenotypeResults2 <- GenotypeResults2[which(GenotypeResults2$A.FDR_adj_genotype <= 0.05),]
R.SignGenotypeResults2 <- GenotypeResults2[which(GenotypeResults2$R.FDR_adj_genotype <= 0.05),]
Array <- c(nrow(A.SignGenotypeResults2))
RRBS <- c(nrow(R.SignGenotypeResults2))

Techtbl <- rbind(Array, RRBS)
Techtbl <- as.data.frame(Techtbl)
colnames(Techtbl) <- c("Genotype")
...
## load rrbs
load("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/biseqResultsPathology_rTg4510.RData")
ls()
rm(DMRsPathology)
# betaresults = array
...
resave(PathologyResults ,file = "/gpfs/ts0/projects/Research_Project-191406/Aisha/data/CompareTech/rTg4510/TechComparison_rTg4510.RData")
