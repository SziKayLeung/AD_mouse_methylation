...
PrepMixedEffectsStats <- function(mixedEffectsResults,mm10_Manifest,intTerm){
  
  mixedEffectsResults <- mixedEffectsResults %>% mutate(cpg = row.names(.))
  mixedEffectsResults <- merge(mixedEffectsResults,mm10_Manifest,by = "cpg")
  mixedEffectsResults <-mixedEffectsResults %>% mutate(position = paste0(seqnames,":",start))
  mixedEffectsResults <- merge(mixedEffectsResults, ChipAnnotatePeaks(mixedEffectsResults), by = "position",suffixes=c("Manifest","ChIPseeker"))
  
  mixedEffectsResults$FDR_adj_GenotypeTG <- p.adjust(mixedEffectsResults[,"PrZ.GenotypeTG"], method = "fdr")
  cat("Number of significant sites in Genotype", nrow(mixedEffectsResults[which(mixedEffectsResults$FDR_adj_GenotypeTG < 0.05),]), "\n") 
  
  if(intTerm == "Tissue.Genotype"){
    ...
    cat("Number of significant sites in Tissue*Genotype", nrow(mixedEffectsResults[which(mixedEffectsResults$FDR_adj_TissueGenotype < 0.05),]), "\n") 
    
  }else if(intTerm == "Geno.Age"){
    mixedEffectsResults$FDR_adj_GenotypeAge <- p.adjust(mixedEffectsResults[,"PrZ.GenotypeTG.Age_months"], method = "fdr")
    mixedEffectsResults <- mixedEffectsResults[order(mixedEffectsResults$PrZ.GenotypeTG.Age_months),]
    cat("Number of significant sites in Tissue*Genotype", nrow(mixedEffectsResults[which(mixedEffectsResults$FDR_adj_GenotypeAge < 0.05),]), "\n") 
    
  }else{
    print("Interaction term required either: <Tissue.Genotype> <Geno.Age>")
  }
