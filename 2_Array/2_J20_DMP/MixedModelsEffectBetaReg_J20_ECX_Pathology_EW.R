...
#subset data to include only entorhinal cortex samples
...
#define function containing interaction model to be used in parallel mode
testCpG<-function(row, phenoJ20_ECX){
  
  library(lmtest)
  
  modelglmm<-glmmTMB(betasJ20_ECX[i,] ~ Pathology + Chip_ID, data=phenoJ20_ECX, REML = FALSE, family=beta_family(link = "probit"))
...
write.csv(res,"/gpfs/ts0/projects/Research_Project-191406/EmmaW/Array/Results/MixedModelResultsECX_Pathology_J20.csv")
