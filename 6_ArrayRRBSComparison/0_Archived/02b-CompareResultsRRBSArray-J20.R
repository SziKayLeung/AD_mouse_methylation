...
png("NumberofFDRSignicantSitesof1233.png")
p<-tableGrob(Techtbl)
grid.arrange(p)
dev.off()
...
png("NumberofFDRSharedSignicantSitesof1233.png")
p<-tableGrob(Techtbl)
grid.arrange(p)
dev.off()
...
if(nrow(A.SignPathologyResults) > 0){
  ...
  pdf("Pathology/ArraySignificantSites.pdf", h = nrow(p))
  rownames(p) <- c(1:nrow(p))
  p<-tableGrob(p)
  grid.arrange(p)
  dev.off()
