...
# Overlaps with list of alzheimers genes from Gemma (AD GWAS 2021 - Bellenguez, +APOE)

```{r load alz gene list from Gemma, warning=F, message=F, echo = F, include = F}

alzGenes <- read.csv("AlzGenes.csv")
human <- unique(alzGenes) #78
names(human) <- "human"
...
There are `r nrow(human)` unique genes in the list of alzheimers genes from Gemma. However 9 of these do not have a mouse analoge, leaving `r length(converted)` genes for comparison
...
```{r AD GWAS RRBS rTg4510 DMPs, warning=F, message=F, echo = F, out.width="50%"}
...
#load in files
temp <- list.files("/gpfs/ts0/projects/Research_Project-191406/EmmaW/RRBSAnnotatedResults/rTg4510/DMPs/", full.names = T)
...
for(i in 1:length(temp)){
  ...
  overlap <- intersect(RRBS_genes, converted)
  overlap_list[[i]] <- sort(overlap)

  
  #populate table
  tab[i, "N.Results"] <- nrow(file)
  tab[i, "N.Genes"] <- length(RRBS_genes)
  tab[i,"N.Overlaps"] <- length(overlap)
...
```{r RRBS rTg4510 AD GWAS vennDiagram, echo=FALSE, include=FALSE}
  
myCol <- c("azure2", "bisque1")

venn.diagram(
  x = list(converted, RRBS_genes),
  category.names = c("AD GWAS", "RRBS"), imagetype = "png",
  filename = "images/RRBS_rTg4510_pathology_gwas.png", main = "RRBS rTg4510 Pathology", fil = myCol, cat.default.pos= "text"
  )
...
![rTg4510 Pathology](images/RRBS_rTg4510_pathology_gwas.png){width=50%}


</center>


List of RRBS rTg4510 genes (genic/tss+/-1500bp) which overlap with the AD GWAS gene list:

```{r AD GWAS RRBS rTg4510 gene list, echo=FALSE}
...
```{r AD GWAS RRBS J20 DMPs, warning=F, message=F, echo = F, out.width="50%"}
...
```{r RRBS J20 AD GWAS vennDiagram, echo=FALSE, include=FALSE}
...
venn.diagram(
  x = list(converted, RRBS_genes),
  category.names = c("AD GWAS", "RRBS"), imagetype = "png",
  filename = "images/RRBS_J20_pathology_gwas.png", main = "RRBS J20 Pathology", fil = myCol, cat.default.pos= "text"
  )
...
![J20 Pathology](images/RRBS_J20_pathology_gwas.png){width=50%}


List of RRBS J20 genes (genic/tss+/-1500bp) which overlap with the AD GWAS gene list:

```{r AD GWAS RRBS J20 gene list, echo=FALSE}
...
```{r AD GWAS Array rTg4510 DMPs, warning=F, message=F, echo = F, out.width="50%"}
...
for(i in 1:length(temp)){
  ...
  #calculate number of overlaps with AD GWAS gene list
...
for(i in 1:length(temp)){
  ...
  #calculate number of overlaps between biomart, chipseeker and either gene list and AD GWAS gene list
...
```{r Array rTg4510 AD GWAS vennDiagram, echo=FALSE, include=FALSE}

myCol <- c("azure2", "bisque1")

venn.diagram(
  x = list(converted, ECX.array_genes_list[[4]]),
  category.names = c("AD GWAS", "Array (ECX)"), imagetype = "png",
  filename = "images/Array_rTg4510_ECX_pathology_gwas.png", main = "Array rTg4510 ECX Pathology", fil = myCol, cat.default.pos= "text"
  )

venn.diagram(
  x = list(converted, HIP.array_genes_list[[4]]),
  category.names = c("AD GWAS", "Array (HIP)"), imagetype = "png",
  filename = "images/Array_rTg4510_HIP_pathology_gwas.png", main = "Array rTg4510 HIP Pathology", fil = myCol, cat.default.pos= "text"
  )
...
knitr::include_graphics(c("images/Array_rTg4510_ECX_pathology_gwas.png", "images/Array_rTg4510_HIP_pathology_gwas.png"))
...
List of array rTg4510 genes which overlap with the AD GWAS gene list:


```{r AD GWAS Array rTg4510  gene list, echo=FALSE}

ECX.geneList <- ECX.makeGeneList(ECX.overlap_list)
...
```{r AD GWAS Array J20 DMPs, warning=F, message=F, echo = F, out.width="50%"}
...
```{r Array J20 AD GWAS vennDiagram, echo=FALSE, include=FALSE}
...
venn.diagram(
  ...
  category.names = c("AD GWAS", "Array (ECX)"), imagetype = "png",
  filename = "images/Array_J20_ECX_pathology_gwas.png", main = "Array J20 ECX Pathology", fil = myCol, cat.default.pos= "text"
  )

venn.diagram(
  x = list(converted, HIP.array_genes_list[[4]]),
  category.names = c("AD GWAS", "Array (HIP)"), imagetype = "png",
  filename = "images/Array_J20_HIP_pathology_gwas.png", main = "Array J20 HIP Pathology", fil = myCol, cat.default.pos= "text"
  )
...
knitr::include_graphics(c("images/Array_J20_ECX_pathology_gwas.png", "images/Array_J20_HIP_pathology_gwas.png"))
...
List of array J20 genes which overlap with the AD GWAS gene list:


```{r AD GWAS Array J20  gene list, echo=FALSE}
...
```{r AD GWAS join RRBS and array gene lists, warning=F, message=F, echo = F}
...
```{r AD GWAS RRBS rTg4510 code vennDiagram, echo=FALSE, include=FALSE}
  
myCol <- c("azure2", "bisque1")

venn.diagram(
  x = list(converted, unionGenes),
  category.names = c("AD GWAS", "Union of genes"), imagetype = "png",
  filename = "images/Union_rTg4510_pathology_gwas.png", main = "Union rTg4510 Pathology", fil = myCol, cat.default.pos= "text"
  )

```
<center>

![Union Pathology](images/Union_rTg4510_pathology_gwas.png){width=50%}


```{r AD GWAS union rTg4510 gene list, echo=FALSE}
...
```{r AD GWAS J20 join RRBS and array gene lists, warning=F, message=F, echo = F}
...
RRBSfiles <- RRBSfiles[-c(3,4,6)] # remove stats table files as we only need the significant results tables, and also results for interaction as there is no array results to pair it with
#read in array ECX files
...
```{r AD GWAS union J20 code vennDiagram, echo=FALSE, include=FALSE}
...
venn.diagram(
  x = list(converted, unionGenes),
  category.names = c("AD GWAS", "Union of genes"), imagetype = "png",
  filename = "images/Union_J20_pathology_gwas.png", main = "Union J20 Pathology", fil = myCol, cat.default.pos= "text"
  )

```
<center>

![J20 Union Pathology](images/Union_J20_pathology_gwas.png){width=50%}



```{r AD GWAS union J20 gene list, echo=FALSE}
