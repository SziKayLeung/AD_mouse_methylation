...
for(clock in clocks){
  ...
  plot(dat2$Age, dat2[, clock], col = dat2$GenotypeCol,
       xlab = "Age", ylab = as.character(clock),
       main = paste("rTg4510 ECX", clock, sep = " - "))
  mtext(paste("cor = ", signif(corr,3)), side = 3, adj = 1)
  abline(0,1)
}
...
for(clock in clocks){
  ...
  plot(dat2$Age, dat2[, clock], col = dat2$GenotypeCol,
       xlab = "Age", ylab = as.character(clock),
       main = paste("rTg4510 HIP", clock, sep = " - "))
  mtext(paste("cor = ", signif(corr,3)), side = 3, adj = 1)
  abline(0,1)
}
...
## DNAmAge and terms
par(mfrow = c(1,3))
plot(dat$Genotype, dat$DNAmAgeClockBrain,
     xlab = "Genotype", ylab = "DNAmAgeClockBrain")
points(dat$Genotype, dat$DNAmAgeClockBrain, col = dat$GenotypeCol)

ages <- sort(unique(dat$Age_months))
means_WT <- c()
means_TG <- c()
for (age in unique(dat$Age_months)) {
  means_WT <- c(means_WT, mean(dat[which(dat$Age_months==age & dat$Genotype=="WT"),"DNAmAgeClockBrain"]))
  means_TG <- c(means_TG, mean(dat[which(dat$Age_months==age & dat$Genotype=="TG"),"DNAmAgeClockBrain"]))
}
plot(dat$Age_months, dat$DNAmAgeClockBrain, col = dat$GenotypeCol, 
     xlab = "Age months", ylab = "DNAmAgeClockBrain")
lines(ages, means_WT, lty=2, col="black")
lines(ages, means_TG, lty=2, col=TG_colour)

plot(dat$Pathology, dat$DNAmAgeClockBrain, col = dat$GenotypeCol, 
     xlab = "Pathology", ylab = "DNAmAgeClockBrain")
abline(lm(DNAmAgeClockBrain ~  Pathology, data=dat))

## Now run the linear models
lmmodel <- lm(DNAmAgeClockBrain ~  Genotype , data=dat)
summary(lmmodel)$coef
...
lmmodel <- lm(DNAmAgeClockBrain ~  Genotype*Age, data=dat)
summary(lmmodel)$coef
...
lmmodel <- lm(DNAmAgeClockBrain ~  Pathology, data=dat)
summary(lmmodel)$coef
...
write.csv(res, "DNAmAge_linearmodelresults.csv")