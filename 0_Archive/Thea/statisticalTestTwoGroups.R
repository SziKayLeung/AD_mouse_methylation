...
# Dorothea Seiler Vellame 07-01-2019 edited on the 19-02-2019

# This test will be appropriate for 2 groups only and includes negative fold change 
...
# groupPheno<-as.factor(phenotype[,4]) 
# 
# RRBSRow<-RRBSm[1,]


statisticalTestTwoGroups<-function(RRBSm,groupPheno){
  ...
  anovaExtract<- function(RRBSRow, groupPheno,RRBSnrow){
    ...
    returnMatrix[3]<-mean(dataAndPheno[1,which(dataAndPheno[2,]==1)])-mean(dataAndPheno[1,which(dataAndPheno[2,]==2)])  # ratio of the means
  ...
  anovaOut<-as.data.frame(apply(RRBSm,1,anovaExtract,groupPheno,3))            # configure output ot desired format
  rownames(anovaOut)<-c("F_value","Degree_of_Freedom","Delta_Beta","p_value","FDR_p", "n_Total")
