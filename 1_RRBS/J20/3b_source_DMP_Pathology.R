...
load(file = paste0(dirnames$processed, "/rrbs/J20_RRBS_SmoothBetas.RData"))

# Remove samples that have NA for pathology 
message("Number of samples: ", nrow(phenotype$J20))
phenotype$J20 <- phenotype$J20[!is.na(phenotype$J20$ECX),]
message("Number of samples after removing samples without pathology data: ", nrow(phenotype$J20))


## ---------- Beta regression -----------------

# common samples 
common_samples <- intersect(row.names(phenotype$J20), colnames(RRBS_smoothbetas))
RRBS_smoothbetas <- RRBS_smoothbetas %>% select(all_of(common_samples))
phenotype$J20 <- phenotype$J20 %>% filter(row.names(.) %in% common_samples) 
...
save(betaResultsClean_Pathology, file = paste0(dirnames$differential, "/rrbs/J20_betaResultsDMP_Pathology.RData"))

# Filter by significance (genotype and interaction effect)
J20_rrbs_DMP_Pathology <- betaResultsClean_Pathology %>% filter(FDR_adj_pathology < 0.05)
save(J20_rrbs_DMP_Pathology, file = paste0(dirnames$differential, "/rrbs/J20_sigResultsDMP_Pathology.RData"))

# merge with genotype results
# significant results
load(paste0(dirnames$differential, "/rrbs/J20_sigResultsDMP_GenotypeInteraction.RData"))
J20_rrbs_DMP$Pathology <- J20_rrbs_DMP_Pathology
save(J20_rrbs_DMP, file = paste0(dirnames$differential, "/rrbs/J20_sigResultsDMPs.RData"))

load(paste0(dirnames$differential, "/rrbs/J20_betaResultsDMP_GenotypeInteraction.RData"))
...
save(betaResultsClean, file = paste0(dirnames$differential, "/rrbs/J20_betaResultsDMPs.RData"))
