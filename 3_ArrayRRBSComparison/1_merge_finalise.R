...
rTg4510_array_beta <- rTg4510_array_beta[!is.na(rTg4510_array_beta$position),] %>% dplyr::select(-Gene_Symbol, -annotation, -cpg) 
rTg4510_array_beta <- as.data.frame(rTg4510_array_beta %>% group_by(position) %>% summarise_all("mean")) %>% tibble::column_to_rownames(., var = "position")
J20_array_beta <- J20_array_beta[!is.na(J20_array_beta$position),] %>% dplyr::select(-Gene_Symbol, -annotation, -cpg) 
J20_array_beta <- as.data.frame(J20_array_beta %>% group_by(position) %>% summarise_all("mean")) %>% tibble::column_to_rownames(., var = "position")
