...
arrayRes <- read.csv("/gpfs/ts0/projects/Research_Project-191406/EmmaW/Array/Results/J20/With_3000bpTSS_Annotations/MixedModelResultsECX_J20_3000tssAnno.csv", stringsAsFactors = F, row.names = 1)
...
RRBSRes <- read.csv("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/DMPs/J20/DMPsInteractionModel_stats_table_J20.csv", stringsAsFactors = F)
...
GenotypeResults <- merge(arrayRes2, RRBSRes2, by = "Position") # 2299
...
There are `r nrow(a)` sites which are significant for the array data and have corresponding locations for the RRBS data. 
...
DMRs <- read.csv("/gpfs/ts0/projects/Research_Project-191406/isabel/RRBS_new/DMRs/J20/DMRsGenotype.csv", row.names = 1, stringsAsFactors = F)
...
None of the 12 significant sites for Genotype for the array data are within a Genotype DMR (n=2)


```{r find overlapping genes between array and RRBS , warning=F, message=F, echo = F, include = F}

#read in annotated RRBS Genotype file
RRBSanno <- read.csv("/gpfs/ts0/projects/Research_Project-191406/EmmaW/Annotated/J20/DMPs/With_3000bpTSS_Annotations/DMPsInteractionModel_J20_sig_genotype_3000tssAnno.csv", stringsAsFactors = F, row.names = 1)
...
## reduce gene annotation to unique genes
#read in file
arrayPathSig <- read.csv("/gpfs/ts0/projects/Research_Project-191406/EmmaW/Array/Results/J20/SigResults/With_3000bpTSS_Annotations/J20_MME_ECX_Genotype_3000tssAnno.csv", stringsAsFactors = F)
...
RRBS_array_overlap <- intersect(array_genes, RRBS_chip_genes) #0
